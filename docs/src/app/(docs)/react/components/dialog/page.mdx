...
- **Backdrop**: render the overlay with `<Dialog.Backdrop>`, which stays in sync with the dialog's open state and transitions. Because a modal popup is a native `<dialog>`, keep the UA `dialog::backdrop` transparent (reset it in your global styles) so it doesn't dim the page on top of `<Dialog.Backdrop>`.
